...
def main():
    ...
    with c2:
        developer_name = st.text_input("Developer / Promoter Name", placeholder="e.g. Er. Anuj Narang")
    ...
    with tab_comm:
        ...
        for j in range(st.session_state.comm_rows):
            ...
            with cc2: c_area = st.number_input(f"Area in Square Yards {j+1}", min_value=0.0, key=f"ca_{j}")
    ...
    # --- PUBLIC SERVICES TAB ---
    with tab_services:
        st.subheader("Public Utilities (STP, Street Light, etc.)")
    ...
    if all_calculated_items:
        ...
        with pd.ExcelWriter(output, engine='xlsxwriter') as writer:
            ...
            headers = ["Sr", "Description", "Norms (kW)", "Qty", "Total kW", "Net Load"]
        ...
        st.download_button("📥 Download Excel Report", output.getvalue(), f"{project_name}_Assessment.xlsx")
